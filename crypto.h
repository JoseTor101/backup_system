...
class SimpleCrypto {
private:
    std::vector<unsigned char> expandKey(const std::string& password, size_t targetLength) {
...
public:
    ...
    std::vector<unsigned char> encrypt(const unsigned char* data, size_t dataLength, const std::string& password = "") {
    ...
    std::vector<unsigned char> decrypt(const unsigned char* encryptedData, size_t dataLength, const std::string& password = "") {
    ...
    std::string encryptString(const std::string& plaintext, const std::string& password = "") {
        auto encrypted = encrypt(reinterpret_cast<const unsigned char*>(plaintext.c_str()), plaintext.length(), password);
        return std::string(encrypted.begin(), encrypted.end());
    }
    
    // Desencriptar string
    std::string decryptString(const std::string& ciphertext, const std::string& password = "") {
        auto decrypted = decrypt(reinterpret_cast<const unsigned char*>(ciphertext.c_str()), ciphertext.length(), password);
